		...
		function SectionFunctions:Tab(Settings)
			...
			function TabFunctions:Section(Settings)
				...
				function SectionFunctions:Progressbar(Settings, Flag)
					...
					local currentValue = math.clamp(Settings.Default or Settings.Value or 0, Settings.Minimum or 0, Settings.Maximum or 100)
					...
					progressbar.Size = UDim2.new(1, 0, 0, 38)
					progressbar.Parent = section

					local progressbarName = Instance.new("TextLabel")
					progressbarName.Name = "ProgressbarName"
					progressbarName.FontFace = Font.new(assets.interFont, Settings.Bold and Enum.FontWeight.Bold or Enum.FontWeight.Regular, Enum.FontStyle.Normal)
					progressbarName.RichText = true
					progressbarName.Text = Settings.Name or ""
					setThemed(progressbarName, "TextColor3", "Text")
					progressbarName.TextSize = 13
					progressbarName.TextTransparency = 0.5
					progressbarName.TextTruncate = Enum.TextTruncate.AtEnd
					progressbarName.TextXAlignment = Enum.TextXAlignment.Left
					progressbarName.TextYAlignment = Enum.TextYAlignment.Center
					progressbarName.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
					progressbarName.BackgroundTransparency = 1
					progressbarName.BorderColor3 = Color3.fromRGB(0, 0, 0)
					progressbarName.BorderSizePixel = 0
					progressbarName.Size = UDim2.new(1, 0, 0, 16)
					progressbarName.Parent = progressbar

					local progressbarValueLabel = Instance.new("TextLabel")
					progressbarValueLabel.Name = "ProgressbarValueLabel"
					progressbarValueLabel.FontFace = Font.new(assets.interFont, Enum.FontWeight.Medium, Enum.FontStyle.Normal)
					progressbarValueLabel.Text = tostring(currentValue)
					setThemed(progressbarValueLabel, "TextColor3", "Text")
					progressbarValueLabel.TextSize = 11
					progressbarValueLabel.TextTransparency = 0.5
					progressbarValueLabel.TextXAlignment = Enum.TextXAlignment.Right
					progressbarValueLabel.TextYAlignment = Enum.TextYAlignment.Center
					progressbarValueLabel.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
					progressbarValueLabel.BackgroundTransparency = 1
					progressbarValueLabel.BorderColor3 = Color3.fromRGB(0, 0, 0)
					progressbarValueLabel.BorderSizePixel = 0
					progressbarValueLabel.Size = UDim2.new(1, 0, 0, 16)
					progressbarValueLabel.Parent = progressbar

					local progressbarUIListLayout = Instance.new("UIListLayout")
					progressbarUIListLayout.Name = "ProgressbarUIListLayout"
					progressbarUIListLayout.Padding = UDim.new(0, 4)
					progressbarUIListLayout.SortOrder = Enum.SortOrder.LayoutOrder
					progressbarUIListLayout.VerticalAlignment = Enum.VerticalAlignment.Center
					progressbarUIListLayout.Parent = progressbar
					...
					trackUICorner.Name = "TrackUICorner"
					trackUICorner.CornerRadius = UDim.new(1, 0)
					trackUICorner.Parent = track

					local trackUIStroke = Instance.new("UIStroke")
					trackUIStroke.Name = "TrackUIStroke"
					trackUIStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
					setThemed(trackUIStroke, "Color", "Outline")
					trackUIStroke.Transparency = 0.9
					trackUIStroke.Parent = track
					...
					fill.BackgroundColor3 = Settings.Color or Color3.fromRGB(73, 230, 133)
					fill.Parent = track

					track.Parent = progressbar
					...
					local function updateFill(val)
						...
						Tween(fill, TweenInfo.new(0.15, Enum.EasingStyle.Sine), {
							Size = UDim2.new(math.clamp(t, 0, 1), 0, 1, 0)
						}):Play()

						local display = val
						if Settings.DisplayMethod == "Percent" then
							display = math.round((val - min) / (max - min) * 100) .. "%"
						elseif Settings.Suffix then
							display = val .. Settings.Suffix
						end
						progressbarValueLabel.Text = tostring(display)
					end

					updateFill(currentValue)

					function ProgressbarFunctions:Set(val)
					...
					function ProgressbarFunctions:Get()
						return currentValue
					end

					function ProgressbarFunctions:SetColor(color3)
						fill.BackgroundColor3 = color3
					end

					function ProgressbarFunctions:UpdateName(New)
						progressbarName.Text = New
					end
...
function WMacLib:Demo()
	...
	local DemoProgressbar = sections.MainSection1:Progressbar({
		Name = "Progressbar",
		Default = 0,
		Minimum = 0,
		Maximum = 100,
