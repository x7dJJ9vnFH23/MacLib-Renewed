	...
	function WindowFunctions:TabGroup()
		...
		function SectionFunctions:Tab(Settings)
			...
			function TabFunctions:Section(Settings)
				...
				function SectionFunctions:Image(Settings, Flag)
					local ImageFunctions = { Settings = Settings }
					...
					imageFrame.Size = UDim2.fromScale(1, 0)
					imageFrame.Parent = section

					local imageFrameUIListLayout = Instance.new("UIListLayout")
					imageFrameUIListLayout.Name = "ImageFrameUIListLayout"
					imageFrameUIListLayout.Padding = UDim.new(0, 6)
					imageFrameUIListLayout.SortOrder = Enum.SortOrder.LayoutOrder
					imageFrameUIListLayout.Parent = imageFrame

					if Settings.Header then
						local imageHeader = Instance.new("TextLabel")
						imageHeader.Name = "ImageHeader"
						imageHeader.FontFace = Font.new(assets.interFont, Enum.FontWeight.Medium, Enum.FontStyle.Normal)
						imageHeader.RichText = true
						imageHeader.Text = Settings.Header
						setThemed(imageHeader, "TextColor3", "Text")
						imageHeader.TextSize = 13
						imageHeader.TextTransparency = 0.4
						imageHeader.TextXAlignment = Enum.TextXAlignment.Left
						imageHeader.AutomaticSize = Enum.AutomaticSize.Y
						imageHeader.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
						imageHeader.BackgroundTransparency = 1
						imageHeader.BorderColor3 = Color3.fromRGB(0, 0, 0)
						imageHeader.BorderSizePixel = 0
						imageHeader.Size = UDim2.fromScale(1, 0)
						imageHeader.Parent = imageFrame
					end
					...
					imageHolder.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
					...
					imageHolder.Size = UDim2.new(1, 0, 0, Settings.Height or 120)
					imageHolder.ClipsDescendants = true
					imageHolder.Parent = imageFrame

					local imageLabel = Instance.new("ImageLabel")
					imageLabel.Name = "ImageLabel"
					imageLabel.Image = resolveImage(Settings.Image)
					imageLabel.ScaleType = Settings.ScaleType or Enum.ScaleType.Crop
					...
					imageUICorner.Name = "ImageUICorner"
					imageUICorner.CornerRadius = UDim.new(0, 6)
					imageUICorner.Parent = imageHolder

					local imageUIStroke = Instance.new("UIStroke")
					imageUIStroke.Name = "ImageUIStroke"
					imageUIStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
					setThemed(imageUIStroke, "Color", "Outline")
					imageUIStroke.Transparency = 0.9
					imageUIStroke.Parent = imageHolder

					function ImageFunctions:SetImage(path)
						imageLabel.Image = resolveImage(path)
					end
					function ImageFunctions:SetHeight(height)
						imageHolder.Size = UDim2.new(1, 0, 0, height)
					end
					function ImageFunctions:SetVisibility(State)
						imageFrame.Visible = State
					end
					function ImageFunctions:Destroy()
						imageFrame:Destroy()
					end

					if Flag then
						WMacLib.Options[Flag] = ImageFunctions
					end
...
function WMacLib:Demo()
	...
	local demoImageId
	if getcustomasset then
		local demoFileName = "Demo.png"
		if not isfile(demoFileName) then
			writefile(demoFileName, game:HttpGet("https://raw.githubusercontent.com/Wicikk/WMacLib/main/Misc/Demo.png"))
		end
		demoImageId = getcustomasset(demoFileName)
	end

	sections.MainSection2:Image({
		Header = "Demo Image",
		Image = demoImageId or "",
		Height = 120,
